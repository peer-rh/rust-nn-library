...
impl Session {
    ...
    pub fn add_node(&mut self, node: Operation) -> Idx {
        ...
        self.nodes.insert(next_idx, node.get_node(next_idx));
        next_idx
    }

    pub fn eval_graph(&mut self, graph: &Graph, feed_dict: Option<HashMap<Idx, f32>>) -> Vec<f32> {
    ...
    pub fn get_value(&self, idx: &Idx) -> f32 {
        self.values[idx]
    }
